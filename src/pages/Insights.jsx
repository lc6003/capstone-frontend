...
export default function Insights() {
  ...
  const lastMonth = useMemo(() => {
    const selectedMonthDate = new Date(now.getFullYear(), selectedMonth, 1)
    const lastMonthDate = new Date(selectedMonthDate.getFullYear(), selectedMonthDate.getMonth() - 1, 1)
    const allExpenses = getExpenses()
  ...
  const getCategoryColor = (index) => {
    const colors = ["#F4A261", "#E07A3F", "#8B5CF6", "#F59E0B", "#10B981", "#EF4444", "#EC4899", "#14B8A6", "#D97706", "#DC2626"]
  ...
  const dailySeries = useMemo(() => {
    ...
    const totals = Array.from({ length: daysInMonth }, (_, idx) => ({ day: idx + 1, value: 0 }))
  ...
  const getComparisonSentence = () => {
    if (lastMonthSpent === 0) {
      if (totalSpent > 0) return t("insights.compare.firstMonthData", "This is your first month with spending data.")
          ...
          {!hasChartData ? (
            ...
            <>
              <div
                ...
                style={{ display: "flex", justifyContent: "center", alignItems: "center", width: "100%", height: "300px" }}
              >
