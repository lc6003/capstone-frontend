...
const monthLocale = locale.startsWith("es") ? "es" : "en"
  ...
  const monthlyData = useMemo(() => {
    ...
    const currentYear = now.getFullYear() // local time
  

    const months = Array.from({ length: 12 }, (_, monthIndex) =>
      new Intl.DateTimeFormat(monthLocale, { month: "short" }).format(
    ...
    return months.map((monthName, monthIndex) => {
      ...
      // spending for this month (exclude Income, normalize negative)
      ...
      // income from budget page (current month only) + income expenses for month
      let income = 0
      ...
      return { month: monthName, monthIndex, income, spend }
    })
  }, [monthLocale])

  // Selected month insights (exclude Income; normalize negative)
  ...
  // Last month insights (month before selected month)
  ...
  // Force re-render when income changes
  const [incomeUpdateTrigger, setIncomeUpdateTrigger] = useState(0)

  useEffect(() => {
    const handleStorageChange = () => setIncomeUpdateTrigger((prev) => prev + 1)
  ...
  // - budget page income entries (current month only)
  // - expenses with category "Income" (selected month)
  ...
  // Summary sentence (not currently displayed in your JSX, kept for future)
  const getSummarySentence = () => {
    ...
    if (isCurrentMonth && currentDay > 0) {
      ...
      if (totalIncome > 0 && projectedSpending > totalIncome) {
        ...
        return t("insights.summary.trendingOverspend", "You're trending to overspend by ${{amt}}.", {
          amt: overspend.toFixed(0)
        })
      }
    }

    if (lastMonthSpent === 0) {
      if (totalSpent > 0) return t("insights.summary.firstMonth", "This is your first month tracking expenses.")
      return t("insights.summary.noneYet", "No expenses recorded yet this month.")
    }

    const absChange = Math.abs(percentChange)
    if (percentChange < 0) {
      return t("insights.summary.lessThanLast", "You're spending {{pct}}% less than last month.", {
        pct: absChange.toFixed(1)
      })
    } else if (percentChange > 0) {
      return t("insights.summary.moreThanLast", "You're spending {{pct}}% more than last month.", {
        pct: absChange.toFixed(1)
      })
    }
  ...
  const sortedCategories = useMemo(() => {
    const categories = Object.entries(byCategory)
      ...
      .map((category, index) => ({ ...category, color: getCategoryColor(index) }))

    return categories
  }, [byCategory, totalSpent])

  // Chart data (absolute values, exclude Income)
  ...
  const selectedMonthDateUTC = new Date(now.getUTCFullYear(), selectedMonth, 1)
  const daysInSelectedMonth = new Date(
    selectedMonthDateUTC.getUTCFullYear(),
    selectedMonthDateUTC.getUTCMonth() + 1,
    0
  ).getUTCDate()
  const isCurrentMonth = selectedMonth === currentMonth
  const currentDayUTC = isCurrentMonth ? now.getUTCDate() : daysInSelectedMonth
  const thisMonthAvgDaily = currentDayUTC > 0 ? totalSpent / currentDayUTC : 0

  const lastMonthDateUTC = new Date(now.getUTCFullYear(), selectedMonth - 1, 1)
  const daysInLastMonth = new Date(
    lastMonthDateUTC.getUTCFullYear(),
    lastMonthDateUTC.getUTCMonth() + 1,
    0
  ).getUTCDate()
  ...
  const getComparisonSentence = () => {
    ...
    if (dollarDiff > 0) {
      return t(
        "insights.compare.up",
        "Spending is up by ${{amt}} (+{{pct}}%) compared to last month.",
        { amt: Math.abs(dollarDiff).toFixed(0), pct: absPercentChange.toFixed(1) }
      )
    } else if (dollarDiff < 0) {
      return t(
        "insights.compare.down",
        "You spent ${{amt}} ({{pct}}%) less than last month.",
        { amt: Math.abs(dollarDiff).toFixed(0), pct: absPercentChange.toFixed(1) }
      )
    }
  ...
  const keyInsights = useMemo(() => {
    ...
    if (topCategory && topCategory[1] > 0) {
      ...
      insights.push({
        icon: FiBarChart,
        text: t(
          "insights.key.topCategory",
          "{{cat}} is your highest spending category this month ({{pct}}%).",
          { cat: topCategory[0], pct: topCategoryPercent.toFixed(0) }
        ),
        type: "info"
      })
    }

    // 2) Top category change vs last month
      ...
      if (lastMonthCategorySpent > 0) {
        ...
        if (Math.abs(categoryChange) >= 10) {
          insights.push({
            ...
            text: t(
              categoryChange > 0
                ? "insights.key.categoryUp"
                : "insights.key.categoryDown",
              categoryChange > 0
    ...
    // 5) Another category change if needed
    if (lastMonthSpent > 0 && insights.length < 4) {
      for (const [categoryName, categorySpent] of Object.entries(byCategory)) {
        ...
        if (lastMonthCategorySpent > 0 && categorySpent > 0) {
          const categoryChange =
            ((categorySpent - lastMonthCategorySpent) / lastMonthCategorySpent) * 100
              ...
              <div className="hero-card-content">
                <div
                  className={`hero-card-icon hero-card-icon-net ${
                    netDifference >= 0 ? "hero-card-icon-positive" : "hero-card-icon-negative"
                  }`}
                >
            ...
            <div className="daily-chart-container">
              <ResponsiveContainer width="100%" height={380}>
                <LineChart data={dailySeries}>
                  ...
                  <XAxis dataKey="day" tickLine={false} axisLine={false} tickFormatter={(day) => day} />
                  <YAxis
                    tickLine={false}
                    axisLine={false}
